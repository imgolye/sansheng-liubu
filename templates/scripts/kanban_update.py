...
def load():
    return _normalize_tasks(atomic_json_read(TASKS_FILE, []))

def save(tasks):
    atomic_json_write(TASKS_FILE, _normalize_tasks(tasks))
    # 异步触发刷新，不阻塞调用方
    try:
...
def next_task_id(prefix=None, date_str=None):
    ...
    for n in range(1, 1000):
        ...
        if task_id not in used:
            return task_id
    raise RuntimeError(f'当天任务号已耗尽: {prefix}-{date_str}-NNN')
